// Modeled on livepeer-network-modules/video-gateway/src/livepeer/rtmp-adapter.ts.
// Verbatim port; renamed file to camelCase for consistency with our other
// livepeer/* files.
...
export async function openRtmpSession(input: OpenRtmpSessionInput): Promise<OpenRtmpSessionOutput> {
  ...
  const routes = await input.routeSelector.select({
    ...
    ...(input.selectionHints !== undefined
      ? {
          preferredExtra: input.selectionHints.preferredExtra,
